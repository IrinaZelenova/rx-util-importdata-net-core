...
{  
  [EntityName("Универсальный передаточный документ")]
  public class IUniversalTransferDocuments : IDocflowAccountingDocumentBases
  {
    new public static IUniversalTransferDocuments FindEntity(Dictionary<string, string> propertiesForSearch, Entity entity, bool isEntityForUpdate, List<Structures.ExceptionsStruct> exceptionList, NLog.Logger logger)
    {
      IUniversalTransferDocuments document = null;
        ...
        int.TryParse(docRegisterId, out int documentRegisterId))
      {
        ...
        document = BusinessLogic.GetEntityWithFilter<IUniversalTransferDocuments>(x => x.RegistrationNumber != null &&
    ...
    new public static IEntityBase CreateOrUpdate(IEntity entity, bool isNewEntity, bool isBatch, List<Structures.ExceptionsStruct> exceptionList, NLog.Logger logger)
    {
      if (isNewEntity)
      {
        var lifeCycleState = ((IUniversalTransferDocuments)entity).LifeCycleState;
        entity = BusinessLogic.CreateEntity((IUniversalTransferDocuments)entity, exceptionList, logger, isBatch);
        return ((IUniversalTransferDocuments)entity)?.UpdateLifeCycleState(lifeCycleState, isBatch);
      }
      else
      {
        return BusinessLogic.UpdateEntity((IUniversalTransferDocuments)entity, exceptionList, logger);
      }
    }
  }
}
